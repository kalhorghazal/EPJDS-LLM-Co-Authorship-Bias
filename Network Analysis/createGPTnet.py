...
for index, row in df.iterrows():
  if pd.isnull(row["Co-authors’ names (GPT 3.5 Turbo)"]):
    continue
  author = row["Name"]
  coauthor_list = row["Co-authors’ names (GPT 3.5 Turbo)"].split("/")
...
for index, row in df.iterrows():
  if pd.isnull(row["Co-authors’ names (GPT 3.5 Turbo)"]):
    continue
  coauthor_list = row["Co-authors’ names (GPT 3.5 Turbo)"].split('/')
  gender_list = row["Co-authors’ genders (GPT 3.5 Turbo)"].split('/')
  ethnicity_list = row["Co-authors’ ethnicities (GPT 3.5 Turbo)"].split('/')
...
nx.write_graphml(G, "gptNet.graphml")